// Android mkbootfs with fs config logic removed, and added chroot and getting permissions from stat for links
...
static void _archive(char *in, char *out, int ilen, int olen)
{
	...
	struct stat s2;
        ...
        buf[size] = 0;
        s.st_mode &=~ 0xfff;
        if(!strcmp(in, "./bin") || !strcmp(in, "./etc") || !strcmp(in, "./first_stage_ramdisk/system/bin/linker_asan64"))
		s.st_mode |= 0x1a4;
        else if(!strcmp(in, "./default.prop"))
	        s.st_mode |= 0x180;
        else if(!strcmp(in, "./init"))
	        s.st_mode |= 0x1e8;
        else if(stat(in, &s2)) {
		s.st_mode |= 0x1a4;
	} else {
	        s.st_mode |= s2.st_mode & 0xfff;
	}
